...
import {
  CSS2DRenderer,
  CSS2DObject,
} from "https://unpkg.com/three@0.150.1/examples/jsm/renderers/CSS2DRenderer.js";
...
let PreviousRoomNumber = parseInt(
...
function addObserverIfDesiredNodeAvailable() {
  ...
  let RoomStyleObseerver = new MutationObserver(function (mutations) {
...
GridDayLight.addEventListener("change", function () {
  ...
  scene.traverse(function (child) {
    ...
    if (child.name === "RH_OUT:grid_daylight") {
      child.visible = GridDayLight.checked;
    }
  });
});
...
GridGlare.addEventListener("change", function () {
  ...
  scene.traverse(function (child) {
    ...
    if (child.name === "RH_OUT:grid_glare") {
      child.visible = GridGlare.checked;
    }
  });
});
...
function collectResults(responseJson) {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    for (const path in values[i].InnerTree) {
      ...
      for (let j = 0; j < branch.length; j++) {
        ...
        if (values[i]["ParamName"] === "RH_OUT:textBasePtX") {
          ...
          textBasePtY = values[i]["InnerTree"]["{0}"][0].data
            .split('"')
            .join("");
        } else if (values[i]["ParamName"] === "RH_OUT:textBasePtZ") {
          textBasePtZ = values[i]["InnerTree"]["{0}"][0].data
            .split('"')
            .join("");
        } else if (rhinoObject !== null) {
  ...
  */
  ...
  const updateAllMaterials = () => {
    ...
    console.log("materials all updated " + i);
    console.log(scene);
  };
 ...
 */
//var bufor = 0
let NumberOfSentences = tipList.length;
let tipsIndex = 0;
...
function showSpinner(enable) {
  ...
  swapSentenceAtIndex(tipList, "tips", tipsIndex);
