...
function collectResults(responseJson) {
  ...
  */
  ...
  var text = new THREE.MeshStandardMaterial({ color: "blue",side: THREE.DoubleSide });
  ...
  loader.parse(buffer, function (object) {
    ...
    object.traverse(child => {
      if (child.name == 'RH_OUT:walls_ext_active') {
      ...
      } else if (child.name == 'RH_OUT:walls_bottom') {
        child.material = (mat_wall_solid);
        ...
        child.material = (text);
        // child.geometry.translate(0,-3.95,0)
        // child.position.x = 0
        // child.position.y = 0
        // child.position.z = 0
        ...
        // const centerX = (boundingBox.max.x - boundingBox.min.x) / 2 + boundingBox.min.x;
        // const centerY = (boundingBox.max.y - boundingBox.min.y) / 2 + boundingBox.min.y;
        // const centerZ = (boundingBox.max.z - boundingBox.min.z) / 2 + boundingBox.min.z;

        // // Translate the mesh to the center of the bounding box
        // child.position.set(-centerX, -centerY, -centerZ);

        child.layers.enableAll();

				 const earthDiv = document.createElement( 'div' );
				 earthDiv.className = 'label3d';
...
// if (Array.isArray(TipsIndex)) {
//   TipsIndex = TipsIndex - 1;
//   bufor = TipsIndex
// } else {
//   // handle the case where the parsed object is not an array
// }
...
function showSpinner(enable) {
  ...
  swapSentenceAtIndex('TipList.json', 'tips', TipsIndex);
  console.log("TipsIndex: " + TipsIndex)
  if (TipsIndex >= NumberOfSentences) {
    TipsIndex = 0;
  }else{
    if (enable){
      TipsIndex++;
...
function swapSentenceAtIndex(jsonFile, divId, index) {
  // Fetch the JSON file and parse it
  fetch(jsonFile)
    .then(response => response.json())
    .then(data => {
      // Get the specified div by ID
      const div = document.getElementById(divId);
      // Get the sentence at the specified index from the JSON data
      const sentence = data[index];
      // Get the <p> tag within the specified div
      const pTag = div.querySelector('p');
      // Replace the text content of the <p> tag with the new sentence
...
//_______________css3d
       // document.body.appendChild( renderer.domElement );
        ...
        // //labelRenderer.domElement.style.z-index = '999'
